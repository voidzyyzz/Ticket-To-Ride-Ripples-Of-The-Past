...
public class LoveTrainEventHandler {
    ...
    private static final double BASE_RADIUS = 0.7;
    ...
    private static final int WATER_CLEAR_INTERVAL = 15*20;

    // ========== 投射物清除参数 ==========
    private static final double PROJECTILE_CLEAR_RANGE = 15.0;

    // ========== 排斥系统参数 ==========
    private static final double REPEL_FORCE = 1.9;
    ...
    public static void onWorldTick(TickEvent.WorldTickEvent event) {
        ...
        while (iterator.hasNext()) {
            ...
            clearHostileProjectiles(event.world, entity, data.getCurrentRadius());
    ...
    // ========== 投射物清除系统 ==========
    private static void clearHostileProjectiles(World world, LivingEntity entity, double radius) {
        double clearRadius = Math.min(radius, PROJECTILE_CLEAR_RANGE);
        AxisAlignedBB area = new AxisAlignedBB(
                entity.getX() - clearRadius,
                entity.getY() - clearRadius / 2,
                entity.getZ() - clearRadius,
                entity.getX() + clearRadius,
                entity.getY() + clearRadius / 2,
                entity.getZ() + clearRadius
        );

        world.getEntitiesOfClass(ProjectileEntity.class, area).forEach(projectile -> {

            Entity shooter = projectile.getOwner();


            if (shooter == null ||
                    (!hasEffect(shooter, InitStatusEffect.LOVE_TRAIN.get()) &&
                            !hasEffect(shooter, InitStatusEffect.LF.get()) &&
                ...
                if (world instanceof ServerWorld) {
                    spawnReflectParticles((ServerWorld) world,
                            entity.position().add(0, 1.5, 0),
                            projectile.position());
                }
            }
        });
